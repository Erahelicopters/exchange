...
(function () {
            ...
            var getHtml = function (feature, attributes) {
                ...
                for (var prop in feature.getProperties()) {
                    if (prop.indexOf('_IDX') == -1 && prop.indexOf('_GEOM') == -1) {
                        if (prop != "geometry") {
                          //ZZ-KMANDALI - 10/30 New changes to check the stationID title in Buoy style info template and make it as a link and open in a new tab when user click on the link
                          //Start
                            if (prop == "TITLE" || prop == "LOCATION_NAME" ||prop == "STATIONID" ) {
                              if(prop == "STATIONID" )
                              {
                                var stationurl = feature.get('STATION_URL');
                                header = "<tr class='" + prop + "'><th colspan=2>" +"<a target='_blank' href="+stationurl+">"+ feature.get(prop) + "</a>"+"</th></tr>";
                              }
                              else{
                ...
                if (typeof selected_.feature.get('features') === 'undefined') {
                    ...
                    if (features.length > 1) {
                        ...
                        for (var i = 0; i < features.length; i++) {
                            var val = features[i].get('TITLE') || features[i].get('LOCATION_NAME') || features[i].get('STATIONID') ;
    ...
    module.controller('viewerController', function ($rootScope, $scope, $q, $injector, $log, $interval, $timeout, stStoryMapBaseBuilder, stAnnotateLayer, MapManager, TimeControlsManager) {
        ...
        $timeout( function(){
            ...
            //krish-11/21 code for adding Weather Buoy layer
            options = {title: "Weather Buoys", server: server, asVector: true, style: buoyStyleFunc, fitExtent: false};
            var buoys = MapManager.addLayer('ERAMAP_NOAA_BUOY', options);

            //kris mandali - 11/21 - Code for adding a metar layer
            options = {title: "Metar Temperature", server: server, asVector: true, style:metarStyleFunc,  fitExtent: false, visible: false};
            var metertemp = MapManager.addLayer('ERAMAP_NOAA_STATION_METAR', options);
            ...
            // krish- 11/16/2017 -  Code for adding layer - Gulf of Mexico
            options = {title: "Heli Gulf of Mexico", server: server, TILED: true , fitExtent: false, visible: false};
            MapManager.addLayer('GOM_32', options);
        ...
        //Filter to get Layers inside the Helli Tiffs LayerList - krish mandali - 11/16- Logc starts
              $scope.layerFilter = function (layer) {
              var title = layer.get('title');
              //console.log(title);
              if(title == 'Heli Gulf of Mexico' ){
              return 1;
            }
              return 0;
            }
        //Filter to get Layers inside the  LayerList
            $scope.layerFilter1 = function (layer) {
            var title = layer.get('title');
            //console.log(title);
            if(title == 'Heli Gulf of Mexico'){
            return 0;
